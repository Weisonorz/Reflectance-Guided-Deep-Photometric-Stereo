...
def customDataloader(args):
    ...
    if args.dataset == 'PS_Synth_Dataset':
        ...
        from datasets.PS_Blobby_BRDF import PS_Blobby_BRDF_Dataset
        train_set = PS_Blobby_BRDF_Dataset(args, args.data_dir, 'train')
        val_set   = PS_Blobby_BRDF_Dataset(args, args.data_dir, 'val')
    else:
        raise Exception('Unknown dataset')

    if args.concat_data:
        assert args.dataset != 'PS_Blobby_BRDF_Dataset', 'Concat data current only support PS_Synth_Dataset'
